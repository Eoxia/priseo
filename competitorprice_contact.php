...
/**
 *  \file       competitorprice_contact.php
 *  \ingroup    priseo
 *  \brief      Tab for contacts linked to CompetitorPrice
 */
...
$hookmanager->initHooks(array('competitorpricecontact', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permission = $user->rights->priseo->competitorprice->write;
} else {
	$permissiontoread = 1;
	$permission = 1;
}
...
$title = $langs->trans('CompetitorPrice')." - ".$langs->trans('ContactsAddresses');
$help_url = '';
...
if ($object->id) {
	...
	print dol_get_fiche_head($head, 'contact', $langs->trans("CompetitorPrice"), -1, $object->picto);
