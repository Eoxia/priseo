...
require_once '../lib/priseo.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "priseo@priseo"));

// Initialize technical object to manage hooks of page. Note that conf->hooks_modules contains array of hook context
$hookmanager->initHooks(array('priseosetup', 'globalsetup'));
...
$arrayofparameters = array(
	...
	//'PRISEO_MYPARAM5'=>array('type'=>'thirdparty_type', 'enabled'=>1),
	//'PRISEO_MYPARAM6'=>array('type'=>'securekey', 'enabled'=>1),
	//'PRISEO_MYPARAM7'=>array('type'=>'product', 'enabled'=>1),
);
...
if ($useFormSetup && (float) DOL_VERSION >= 15) {
	...
	// Setup conf PRISEO_MYPARAM1 as a simple string input
	$item = $formSetup->newItem('PRISEO_MYPARAM1');

	// Setup conf PRISEO_MYPARAM1 as a simple textarea input but we replace the text of field title
	$item = $formSetup->newItem('PRISEO_MYPARAM2');
	$item->nameText = $item->getNameText().' more html text ';

	// Setup conf PRISEO_MYPARAM3
	$item = $formSetup->newItem('PRISEO_MYPARAM3');
	$item->setAsThirdpartyType();

	// Setup conf PRISEO_MYPARAM4 : exemple of quick define write style
	$formSetup->newItem('PRISEO_MYPARAM4')->setAsYesNo();

	// Setup conf PRISEO_MYPARAM5
	$formSetup->newItem('PRISEO_MYPARAM5')->setAsEmailTemplate('thirdparty');

	// Setup conf PRISEO_MYPARAM6
	$formSetup->newItem('PRISEO_MYPARAM6')->setAsSecureKey()->enabled = 0; // disabled

	// Setup conf PRISEO_MYPARAM7
	$formSetup->newItem('PRISEO_MYPARAM7')->setAsProduct();
	*/
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/priseo/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
			$filefound = 1;
	...
	if ($filefound) {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=priseo-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'PRISEO_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		$tmpobjectkey = GETPOST('object');
		if (!empty($tmpobjectkey)) {
			$constforval = 'PRISEO_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
print dol_get_fiche_head($head, 'settings', $langs->trans($page_name), -1, "priseo@priseo");
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'PRISEO_'.strtoupper($myTmpObjectKey).'_ADDON';
