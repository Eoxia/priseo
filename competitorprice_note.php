...
/**
 *  \file       competitorprice_note.php
 *  \ingroup    priseo
 *  \brief      Tab for notes on CompetitorPrice
 */
...
$langs->loadLangs(array("priseo@priseo", "companies"));
...
$hookmanager->initHooks(array('competitorpricenote', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->priseo->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
$help_url = '';
llxHeader('', $langs->trans('CompetitorPrice'), $help_url);
...
if ($id > 0 || !empty($ref)) {
	...
	print dol_get_fiche_head($head, 'note', $langs->trans("CompetitorPrice"), -1, $object->picto);
