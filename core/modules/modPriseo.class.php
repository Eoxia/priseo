 ...
 *
 *  \file       htdocs/priseo/core/modules/modPriseo.class.php
 *  \ingroup    priseo
...
class modPriseo extends DolibarrModules
{
	...
	public function __construct($db)
	{
		...
		$this->description = "PriseoDescription";
		...
		$this->module_parts = array(
			...
			'css' => array(
				//    '/priseo/css/priseo.css.php',
			),
			// Set this to relative path of js file if module must load a js on all pages
			'js' => array(
				//   '/priseo/js/priseo.js.php',
			),
		...
		$this->dirs = array("/priseo/temp");

		// Config pages. Put here list of php page, stored into priseo/admin directory, to use to setup module.
		$this->config_page_url = array("setup.php@priseo");
		...
		$this->tabs = array();
		// Example:
		$this->tabs[] = array('data'=>'product:+priseo:CompetitorPrice:priseo@priseo:$user->rights->priseo->competitorprice->read:/priseo/competitorprice_card.php?id=__ID__');  					// To add a new tab identified by code tabname1
		...
		*/
		...
		/*$this->menu[$r++] = array(
			...
			'mainmenu'=>'priseo',
			'leftmenu'=>'',
			'url'=>'/priseo/priseoindex.php',
			'langs'=>'priseo@priseo', // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000 + $r,
			'enabled'=>'$conf->priseo->enabled', // Define condition to show or hide menu entry. Use '$conf->priseo->enabled' if entry must be visible if module is enabled.
		...
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=priseo',      // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type'=>'left',                          // This is a Left menu entry
			'titre'=>'CompetitorPrice',
			'prefix' => img_picto('', $this->picto, 'class="paddingright pictofixedwidth valignmiddle"'),
			'mainmenu'=>'priseo',
			'leftmenu'=>'competitorprice',
			'url'=>'/priseo/priseoindex.php',
			'langs'=>'priseo@priseo',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->priseo->enabled',  // Define condition to show or hide menu entry. Use '$conf->priseo->enabled' if entry must be visible if module is enabled.
			'perms'=>'$user->rights->priseo->competitorprice->read',			                // Use 'perms'=>'$user->rights->priseo->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=priseo,fk_leftmenu=competitorprice',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type'=>'left',			                // This is a Left menu entry
			'titre'=>'List_CompetitorPrice',
			'mainmenu'=>'priseo',
			'leftmenu'=>'priseo_competitorprice_list',
			'url'=>'/priseo/competitorprice_list.php',
			'langs'=>'priseo@priseo',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->priseo->enabled',  // Define condition to show or hide menu entry. Use '$conf->priseo->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=>'$user->rights->priseo->competitorprice->read',			                // Use 'perms'=>'$user->rights->priseo->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		$this->menu[$r++]=array(
			'fk_menu'=>'fk_mainmenu=priseo,fk_leftmenu=competitorprice',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
			'type'=>'left',			                // This is a Left menu entry
			'titre'=>'New_CompetitorPrice',
			'mainmenu'=>'priseo',
			'leftmenu'=>'priseo_competitorprice_new',
			'url'=>'/priseo/competitorprice_card.php?action=create',
			'langs'=>'priseo@priseo',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'$conf->priseo->enabled',  // Define condition to show or hide menu entry. Use '$conf->priseo->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
			'perms'=>'$user->rights->priseo->competitorprice->write',			                // Use 'perms'=>'$user->rights->priseo->level1->level2' if you want your menu with a permission rules
			'target'=>'',
			'user'=>2,				                // 0=Menu for internal users, 1=external users, 2=both
		);
		*/
       ...
       /* $this->menu[$r++]=array(
            ...
            'type'=>'left',
            'titre'=>'List CompetitorPrice',
            'mainmenu'=>'priseo',
            'leftmenu'=>'priseo_competitorprice',
            'url'=>'/priseo/competitorprice_list.php',
            // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'langs'=>'priseo@priseo',
            'position'=>1100+$r,
            // Define condition to show or hide menu entry. Use '$conf->priseo->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
            'enabled'=>'$conf->priseo->enabled',
            // Use 'perms'=>'$user->rights->priseo->level1->level2' if you want your menu with a permission rules
            'perms'=>'1',
            'target'=>'',
        ...
        $this->menu[$r++]=array(
            // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
            'fk_menu'=>'fk_mainmenu=priseo,fk_leftmenu=priseo_competitorprice',
            // This is a Left menu entry
            'type'=>'left',
            'titre'=>'New CompetitorPrice',
            'mainmenu'=>'priseo',
            'leftmenu'=>'priseo_competitorprice',
            'url'=>'/priseo/competitorprice_card.php?action=create',
            // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
	...
	public function init($options = '')
	{
		...
		//$result = $this->_load_tables('/install/mysql/tables/', 'priseo');
		...
		//$result1=$extrafields->addExtraField('priseo_myattr1', "New Attr 1 label", 'boolean', 1,  3, 'thirdparty',   0, 0, '', '', 1, '', 0, 0, '', '', 'priseo@priseo', '$conf->priseo->enabled');
		//$result2=$extrafields->addExtraField('priseo_myattr2', "New Attr 2 label", 'varchar', 1, 10, 'project',      0, 0, '', '', 1, '', 0, 0, '', '', 'priseo@priseo', '$conf->priseo->enabled');
		//$result3=$extrafields->addExtraField('priseo_myattr3', "New Attr 3 label", 'varchar', 1, 10, 'bank_account', 0, 0, '', '', 1, '', 0, 0, '', '', 'priseo@priseo', '$conf->priseo->enabled');
		//$result4=$extrafields->addExtraField('priseo_myattr4', "New Attr 4 label", 'select',  1,  3, 'thirdparty',   0, 1, '', array('options'=>array('code1'=>'Val1','code2'=>'Val2','code3'=>'Val3')), 1,'', 0, 0, '', '', 'priseo@priseo', '$conf->priseo->enabled');
		//$result5=$extrafields->addExtraField('priseo_myattr5', "New Attr 5 label", 'text',    1, 10, 'user',         0, 0, '', '', 1, '', 0, 0, '', '', 'priseo@priseo', '$conf->priseo->enabled');
		...
		// Document templates
		$moduledir = dol_sanitizeFileName('priseo');
		$myTmpObjects = array();
		$myTmpObjects['CompetitorPrice'] = array('includerefgeneration'=>0, 'includedocgeneration'=>0);

		foreach ($myTmpObjects as $myTmpObjectKey => $myTmpObjectArray) {
			if ($myTmpObjectKey == 'CompetitorPrice') {
				continue;
			}
			if ($myTmpObjectArray['includerefgeneration']) {
				$src = DOL_DOCUMENT_ROOT.'/install/doctemplates/'.$moduledir.'/template_competitorprices.odt';
				$dirodt = DOL_DATA_ROOT.'/doctemplates/'.$moduledir;
				$dest = $dirodt.'/template_competitorprices.odt';
